        ...
        with tf.Session() as sess:
            ...
            for i, organ in enumerate(organ_list):
                ...
                with tf.device('/cpu:0'):
                    test_data = ImageDataGenerator_testset(dir_path=dir_path, img_resize=resize_shape, channel=channel, num_patient=1, pix_upper_lim=pix_upper_lim, pix_lower_lim=pix_lower_lim)
...
def mask_color_img(img, pred, color_list, itr, alpha=0.6):
    ...
    for slice_num in range(img.shape[0]):
        ...
        for organ_num, organ_pred in enumerate(pred):
            ...
            print("pred_slice : ", np.max(pred_slice.flatten()))
        ...
        print("img_pred : ", np.max(img_pred.flatten()))
