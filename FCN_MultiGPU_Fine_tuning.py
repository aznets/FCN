...
class FCN_MultiGPU_FineTuning:
    def __init__(self, num_gpu, mode, logs_dir, dir_path, mask_name):
    ...
    def dice(self, mask1, mask2, smooth=1e-6):
        print("-"*50)
    ...
    def main(self, learning_rate, optimization, batch_size, top_loss, top_loss_name, total_loss, total_loss_name, pixel_range):
        ...
        self.fname = str(self.learning_rate) + '_' + self.optimization + '_' + str(self.batch_size) + '_' + str(pixel_range)
        ...
        # Need to add another mode to draw the contour based on image only.
            ...
            for index in range(test_index):
                ...
                for itr in range(test_batch_size):
                    ...
                    plt.title("pred mask")
                    plt.savefig(self.logs_dir + "/Prediction_test" + str(ind) + ".png")
            ...
            with tf.device('/cpu:0'):
                vd_data = ImageDataGenerator(dir_path=self.dir_path, mask_name=self.mask_name, img_resize=resize_shape, mask_resize=resize_shape, channel=channel, mode='validation',
                                             ...
                                             pix_upper_lim=pixel_range[1], pix_lower_lim=pix_range[0])
...
if __name__ == "__main__":
    ...
    # Discrete variables
    # 1. Loss function
    loss_fn = 'dice' # 'cross_entropy'  # 'dice' or 'cross_entropy'
    # 2. batch_size per GPU
    batch_size = [32] #[16, 32]
    
    # 3. range of pixel(lower/upper pixel intensity) 
    pixel_range = [(-1000,3000),(-500,500),(-200,300)] # default was (-1000, 3000)

    pr_b_comb = [(x,y) for x in pixel_range for y in batch_size]

    # Continuous variables
    # 1. Learning Rate lr, total run = 5*len(batch_size)
    rnd_num = 4.0*np.random.rand(1, 10*len(pixel_range)*len(batch_size)) + 2.0  # range (2.0, 6.0) so the range of learning rate to be (10^-2 , 10^-6)
    ...
    num_gpu = 4
    logs_dir = "logs_mandible_FineTuning"
    ...
    for i, l in enumerate(lr):
        ...
        FineTuning = FCN_MultiGPU_FineTuning(num_gpu=4,mode='train',logs_dir=logs_dir, dir_path=dir_path, mask_name=mask_name)
    ...
    #REMOVE IF thing above works!
    print(total_loss)
    print(total_loss_name)
